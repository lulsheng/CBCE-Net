...
class Prec(object):
    ...
    def get_results(self) -> dict:
        ...
        :return: dict(mae=mae)
        """
        IoUs = np.array(self.IoUs, _TYPE)
        prec_05 = (sum(IoUs>0.5)) / len(IoUs)
        prec_06 = (sum(IoUs>0.6)) / len(IoUs)
        prec_07 = (sum(IoUs>0.7)) / len(IoUs)
        prec_08 = (sum(IoUs>0.8)) / len(IoUs)
        prec_09 = (sum(IoUs>0.9)) / len(IoUs)
        res = {'prec_0.5': prec_05, 'prec_0.6':prec_06, 'prec_0.7':prec_07, 'prec_0.8':prec_08,'prec_0.9':prec_09}
        return res
